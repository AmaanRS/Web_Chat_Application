...
class SocketLogic {
  ...
  initListeners() {
    ...
    io.use(async (socket, next) => {
      try {
        ...
        if (doesExist) {
          return next(
            new Error("One instance of this email is already logged in")
          );
        }
        next();
      } catch (error) {
    ...
    io.on("connect", async (socket) => {
      ...
      socket.on("disconnect", async (reason) => {
        console.log(`Socket ${socket.id} disconnectedddd. due to ${reason}`);

        //Delete the value from redis on disconnection
        if (socket.email) {
          console.log("Clean Ran in disconnect");
      ...
      socket.on("event:logout", async (data) => {
        try {
          ...
          console.log("Clean ran in logout");
  ...
  async cleanUserFromRedis(email) {
    ...
    console.log("userFriendsOnline", userFriendsOnline);
